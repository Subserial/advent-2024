...
fn parse(data: &str) -> (Vec<(u32, u32)>, Vec<Vec<u32>>) {
    ...
    let orders = first.lines().map(|line| {
        let mut vals = line.split('|').map(|n| n.parse::<u32>().unwrap());
        (vals.next().unwrap(), vals.next().unwrap())
    }).collect();
    let sequences = second.lines().map(|line| {
        line.split(',').map(|n| n.parse::<u32>().unwrap()).collect::<Vec<_>>()
    }).collect();
    (orders, sequences)
}
...
pub fn run_one(data: &str) -> String {
    ...
    let good = extract_good(&orders, &sequences).into_iter().collect::<HashSet<_>>();
    sequences.iter().enumerate().filter(|(i, _)| good.contains(i)).map(|(_, sequence)| sequence[sequence.len() / 2]).sum::<u32>().to_string()
}

pub fn run_two(data: &str) -> String {
    let (orders, sequences) = parse(data);
    let good = extract_good(&orders, &sequences).into_iter().collect::<HashSet<_>>();
    sequences.into_iter().enumerate().filter(|(i, _)| !good.contains(i)).map(| (_, mut sequence)| {
